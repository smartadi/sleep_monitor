...
- **2026-09-11** — **Four analyses answering the professor's Sep 7–9 manuscript emails** (`analysis/prof_requests_sep2026/{mean_cap_vs_rem,band_power_comparison,cre_vs_cle_laterality,kcomplex_comparison}.py`; plots `notebooks/plots/prof_requests_sep2026/`; tables `reports/prof_requests_sep2026/`; summary `analysis/prof_requests_sep2026/FINDINGS.md`). Descriptive, per-subject, no headline p-values ([[feedback_reporting_simple]]). **(A) Mean cap vs REM/sleep cycle** — the slow mean *does* swing +/- over the night (prof's observation is real) but the excursions are **step-like, posture/coupling-dominated, not tightly locked to the NREM–REM cycle**: correlation of slow mean with a smoothed REM-occupancy cycle is weak (median |r| ≈ 0.10–0.15, every channel). Modest consistent direction: **CH higher in REM in 9/12 nights** (+0.19 z), **CLE−CRE lower in REM in 8/12** (−0.33 z); REM-onset-triggered average weak pooled. **(B) Band power** (median fF², motion-free sleep): **CH dominates every band** (resp 0.153, card 0.107, SWS-N3 0.060 vs CLE/CRE ~0.01–0.07); 1/f falloff so resp > cardiac/SWS; **harmonic-comb epochs carry ~3–5× the 0.1–3 Hz power of ordinary NREM** (CH 2.92 vs 0.27). PSD figure with bands shaded = "see the band power". **(C) CRE vs CLE laterality — not one uniform answer:** persistent **respiratory ridge bilateral** (CLE 96% ≈ CRE 97%, LI≈0 all 12 nights); **cardiac ridge left(CLE)-dominant** (21% vs 12%, 8/12 nights); **N3 broadband SWS-prominence ridge left-dominant** (~18% vs 4%); **harmonic combs one-sided right/CH, essentially absent on CLE** (0.06% vs 1.2/1.4%) but rare and N2, per-night laterality noisy (concentrated in S6). **(D) K-complex** — the N2 delta-onset set *is* the K-complex/isolated-slow-wave population; re-cut to N2 onsets (n=340, 6 subjects), the **SEC band-power response FOLLOWS the K-complex, peaking +2 to +5 s** (CLE ~2 s, CH ~4 s, CRE ~5 s), all 3 bands × 3 channels above the random-NREM null, **no precursor**. This is the **Fultz et al. 2019 connection** the prof flagged (EEG leads the downstream CSF/hemodynamic response ~6 s); reproduces/extends [[project_delta_onset_precursor]] (CAP response after, not before, the cortical event). D reuses `analysis/delta_onset/delta_cap_precursor.py` machinery; C reuses `reports/slow_wave/{band_ridge_epochs,overlay/ridge_overlay_epochs,ladder_quantify/per_window_channels}.parquet`. **Still open (document work, not analysis):** redraw manuscript Fig 3/4/8/9/10 larger font (+ (a)/(b) on Fig 10); supplementary Fig S4 background + font; fix duplicate S3/S5/S6 numbering; read the Fultz PDF into §4.5.
- **2026-08-17** — **CAP arousal events — an inventory** (`analysis/swa_validation/cap_arousal_events.py`; tables `reports/swa_validation/cap_events/`; figures `writeup/figures/cap_events/`). **Request (user):** report events, do not try to prove anything statistically — find CAP variations that are NOT head movement and ask whether they line up with EEG events. **Design note recorded because it shapes every number below:** "CAP moved, EEG did not" is an argument from absence and cannot stand alone — an unexplained CAP transient is indistinguishable from sensor noise. Every event is therefore also read against ECG and Pleth, which are neither EEG nor the accelerometer, so a non-cortical event can be *positively* identified as an arousal rather than merely inferred from the EEG's silence. **Counts:** 1,301 CAP transients detected over 12 nights; **319 (25%) survive** the four artifact gates (head motion, head turn, level step, impulse). Kept rate **1.1–9.8 per hour** (median ≈ 2.8). **Class composition of the 319: 57 (18%) coincide with a scored arousal; 64 (20%) show an unscored EEG 8–30 Hz rise; 124 (39%) show no EEG change but a heart-rate surge and/or a pulse-amplitude drop; 74 (23%) show nothing anywhere.** **The most important structural finding is about the motion gate, not the classes:** **493/1,301 (38%) of all detections overlap a scored arousal, but only 57/319 (18%) of the kept ones do** — of the 436 concordant detections that were rejected, **409 were rejected for head motion**. Scored arousals are movement-coupled almost by definition, so requiring "not head movement" removes most of the events where CAP and EEG agree. Class 1 is therefore small **by construction, not by failure**, and the 38% pre-rejection concordance is the honest statement that the detector finds real arousals. **Morphology (median peri-event traces, descriptive only, no test):** class 3 shows a CAP burst with a flat EEG, HR rising ≈ +1 bpm and holding for ~40 s, and pulse amplitude dipping after onset — the autonomic signature of an arousal without a cortical one. **Class 4 is the control that makes the rest readable:** same CAP burst, EEG flat, HR drifting *down*, pulse amplitude drifting *up* — no arousal signature at all, i.e. the criteria are not simply firing on any transient. **Stage distribution:** events concentrate in N2 (196/319) with few in REM (9). **Impulse gate:** added specifically to test whether the CAP bursts are single-sample electrical glitches; it removes 27 detections and only **1** kept event, so they are not. **Status: HELD — deliberately not in the paper** (user decision, 2026-08-17: the manuscript has enough without it). Inventory is complete on all 12 sessions and fully reproducible from the committed script; no inferential claim is attached to any number here. Do not treat this as pending manuscript work. Related: [[project_cortical_arousal_events]], and the epoch-level precursor work in `cap_variance_vs_cortical.py`.
- **2026-08-17** — **Ridge + harmonic-ladder analysis, fully revamped** (`analysis/slow_wave/{ridge_overlay_tune,harmonic_ladder_overlay,ladder_zoom}.py`; module upgrades in `sleep_monitor/harmonics.py`; figures `writeup/figures/harmonics/{ridges,ladders}/`; report `reports/slow_wave/revamp/harmonic_ladders_long.csv`). **Request (user):** the paper's ridge/harmonic analysis is stale — redo it; make long ridges flat, harmonic detection strong, add a slow (~0.1 Hz) band; then the ladder is a *distinct* phenomenon (candidate SWA signature), NOT the resp/cardiac harmonics the ridges already report — detect it as a temporally-flat, structured, single concept, per channel. **RIDGES (`ridge_overlay_tune.py`):** per channel, respiration and heart rate tracked as ONE smooth continuous line each by a **Viterbi ridge tracker** (globally-optimal path, no per-window hopping; resp band 0.25–0.55 penalty 20, cardiac 0.85–1.80 penalty 18) — fixed the earlier jitter and cardiac→subharmonic locking; plus a **slow band 0–0.3 Hz** (multi-ridge). Also added `fill_gaps`+iterative-merge+`dominant_only` to `detect_persistent_ridges`. **LADDER — the hard part, resolved by INSTRUMENTING not tuning:** after a long thrash of hand-tuned gates, tested candidate discriminators on the visible S6N1 episodes: a "principled" spectral-autocorrelation comb-strength did NOT separate episode from baseline (episode z≈0, comb is a small fraction of total spectral variance); a **dB sweep of the bright-rung count DID** — at RUNG_DB=5 (peak ≥5 dB above the per-column background) episodes carry ~5 rungs vs baseline 0 (at 4 dB noise floods every window). Final method (per channel, kept separate — averaging diluted single-electrode episodes): bright-rung count sets episode EXTENT (drop short/isolated noise cores before gap-bridging; hysteresis-extend), then RUNGS = actual peaks of the time-averaged episode spectrum drawn as flat lines at their TRUE (non-uniform) frequencies, count auto-detected. **KEY FINDINGS:** (1) flat harmonic-comb episodes are real and reproducible (e.g. S6N1 CH two episodes 2.25–2.7 & 3.85–4.05 h, 5 rungs at 0.15/0.28/0.42/0.68/0.95 Hz — quasi-harmonic, NOT integer-spaced); detected on all three channels where present, quiet elsewhere. (2) **They occur predominantly in N2, NOT N3** (11/12 detections N2/N1; only S3N2/CH is N3) — so the "intracranial SWA signature" framing does NOT hold as N3-specific in this cohort; the ladder is an N2-dominant phenomenon. (3) The resp/cardiac ridges are continuous flat single lines per channel; slow-band (0–0.3 Hz) ridge present. **Takeaway:** clean, principled ladder detector (one grounded feature + actual-peak rungs) replaces the fragmented old story; the ladder is a distinct N2 comb episode, not cortical SWA. Stage ladder ordered Wake·N1·N2·N3·REM per user. Pending: fold into manuscript §3.6/§4.3 (not yet done); reconcile with the prior [[project_slow_wave_analysis]] framing (which claimed N3 SWA — now contradicted for the ladder).
- **2026-08-14** — **Rate detection: code review, full rerun on a fixed estimator, and consolidation of every rate claim in the repo** (`analysis/rates/rerun_rate_detection.py`, `rerun_figures.py`, `rerun_session_plots.py`, `k_age_rerun.py`; new `sleep_monitor/rates.py::rate_spectral_interp`; outputs `reports/rates/rerun/*`, `artifacts/rate_rerun_phase_a.parquet` 93,190 rows; figures `writeup/figures/rate_rerun/*`). **Request (user):** review the rate code, rerun it, redo the paper figures, then edit the paper.

  **(1) Two defects found in code review, and the second was new.** `rate_spectral` (rates.py:32) fixes the Welch segment at 4 s, so Δf = 0.25 Hz whatever the band. The 0.1–0.5 Hz respiratory band holds two usable bins and the estimator returns **15.0 br/min in 99.98% of epochs**, zero within-session variance in 10/12 sessions; k is then identically `15/median(reference)`, so the "calibrated respiratory rate" was each session's own median reference rate and the published **0.91 br/min was the error of the best constant**. Cardiac is quantized by the same Δf to 15 BPM steps. The 2026-08-06 audit had this (A.1). **What the audit missed:** the fix already in the manuscript does not fix it — for respiration `fused_agree` is `smart_fuse_two(r_spectral, r_peaks_loose)` (`run_mask_rate_detection.py:267`), so the degenerate estimator is an *input* to the pipeline being reported as non-degenerate. Pooled SDs: `diff_spectral` 0.22 (2 unique values), **`fused_agree` 0.46** (6,897 unique), `diff_peaks_loose` 2.89, against a reference SD of 1.57. A diluted constant.

  **(2) Fixing the resolution makes it worse — and that is the real finding.** `rate_spectral_interp` uses the whole window as one Hann segment (Δf = 1/T = 0.033 Hz), zero-pads 8× and parabolically interpolates the peak; on synthetic tones it recovers frequency to ~2×10⁻⁴ Hz. On real data it is **worse**: SD 4.2–5.2 br/min and error 3.9–4.4, because the respiratory rhythm is often not the largest peak in the band. Interpolation refines *which* peak you found, not whether it is the right one. Deeper constraint: within-night respiratory SD is 1.57 br/min = 0.026 Hz, needing a ≥40 s segment to resolve; a 30 s epoch gives Δf = 2.0 br/min at best. **30 s cannot support spectral respiratory estimation at all.** Best genuinely varying estimator is loose peak counting on CRE, **1.79 br/min**.

  **(3) Nothing survives held-out calibration.** Ran the test the original analysis never did — self-k / cross-night k / leave-one-**subject**-out population k / a no-sensor comparator predicting the LOSO cohort median. Night-mean error, paired Wilcoxon on 12 nights: respiratory self-k 0.24 (11/12, p=0.001) but cross-night 0.57 (8/12, **p=0.110**) and population 0.94 (5/12, p=0.301) against **no-sensor 1.20**; cardiac self-k 1.56 (p=0.077), cross-night 3.77 (5/12, **p=0.850**), population 3.19 (p=0.622) against no-sensor 2.76. Epoch level is *worse* than no-sensor in every regime, some significantly. Only same-night calibration beats the baseline, and that is a fitting residual. Confirms audit A.2 on a non-degenerate estimator.

  **(4) k is session-wide, and the k–age finding is dead.** Confirmed from code that k is one scalar per (session, band, estimator) — the median of estimate÷reference over **every** valid epoch, ratios clipped 0.3–5.0 (`run_mask_rate_detection.py:397`). The **"50 randomly drawn one-minute windows"** in §3.5/§4.2.1/§5.1 describes no code that produced any reported number. On the operational estimator the respiratory age relationship collapses: **ρ = −0.37, exact p = 0.50**, leave-one-out ρ spans −0.90 to **+0.10** so the sign is not stable, and a held-out age prior (0.164) is **worse** than a single constant (0.115). The published ρ = −0.83 was an artifact of the constant estimator — "k declines with age" only ever said "median breathing rate rises with age in this cohort". At n = 6, |ρ| ≥ 0.83 is needed for p < 0.05. Cardiac unchanged: ρ = +0.26, p = 0.66, unstable, prior loses to the constant. **Survives:** cardiac k = 1.96 [1.77–2.01] still bracketed by 1.70–2.43 peaks per beat (median 2.02 CRE, 9 sessions), and the within-night negative (median r −0.03 / −0.08; 0/12 and 1/12 nights above a circular-shift null). Respiratory k is **1.18**, not ≈1, so "one dominant temple displacement per breath" is wrong.

  **(5) Paper.** §4.2 reframed from validation to **demonstration** per user direction — agreement on 12 nights with per-session calibration, conditionality stated in one sentence, held-out analysis out of the main text. Table 3 recomputed and rebuilt (the original was a 13-row grid with blank cells; a flat cell-fill shifted every label off its values). Figure 4 → representative night, Figure 5 → Bland–Altman, S8/S9 → all twelve nightly traces. k–age analysis and its figure deleted; downstream numbers fixed in §5.1, §6 and §7.

  **(6) Consolidation.** `analysis/rates/CLAUDE.md` rewritten as the authoritative current state with a Traps section and an explicit superseded list; `writeup/paper/KEY_NUMBERS.md` rate sections replaced; `sleep_monitor/rates.py` docstring carries a warning on `rate_spectral`; root `CLAUDE.md` estimator count corrected; SUPERSEDED banners added to `writeup/paper/CLAIMS.md`, `writeup/edits/RATE_AUDIT_2026-08-06.md` and the four `notebooks/*writeup*.md` / `validation_*.md` files.

- **2026-08-06** — **Paper figure set for the delta-onset result, and a motion control that overturns it** (`analysis/delta_onset/delta_onset_figures.py`; figures → `writeup/figures/delta_onset/fig_delta_onset_{cohort,motion_control,session,subjects,age}.png`, CSVs `delta_onset_{response_summary,timing_calibration,age_correlations}.csv`). Asked for clean all-band figures at cohort / single-session / per-subject / age level, plus a check on theoretical accuracy. The figures were straightforward; the accuracy check was not.

  **(1) The CAP event at delta onset is head movement.** The detector screened onsets for a motion-clean *pre*-onset window; nothing ever constrained the post-onset window, which is where the response sits. Post-onset motion flags run at **14.6 % of samples on average (3.0–31.6 % per session)** against ~1 % before onset, peaking at **~30 % at +4 s** — the same place the CAP peak sits. Restricting to the **150 of 339 onsets that are also motion-clean after t = 0**, against a null carrying the identical post-window gate, the response is **absent in all nine channel × band combinations**: real − null **−0.045 to −0.170 z**, with **0–3 of 5 subjects positive** (i.e. at or below chance). Contamination is worst exactly where the data is densest — S2 supplied **179 of 339 onsets** and is the most contaminated night (27–32 % post-onset motion; only 26/99 and 9/80 onsets clean), so the cohort average was largely S2's movements.

  **(2) The delta bursts themselves are genuine — the detector is not firing on EEG movement artifact.** Motion-clean onsets still carry an EEG delta peak of **2.47 z** (vs 3.13 z for all onsets; null −0.30 z either way). So these are real cortical bursts with no capacitive correlate unless the head moves. That kills the §4.5 mechanism as written: "a mechanical/hemodynamic co-activation … amplitude modulation of the ongoing respiratory/cardiac signals" predicts a response on movement-free onsets. The remaining honest reading is either a movement artifact, or the mask registering K-complex-associated motor activation — and these data cannot separate those two.

  **(3) Latencies were upper bounds, not estimates.** Measured each estimator's bias on a synthetic burst that truly starts at t = 0: the causal envelope reports **+1.90 / +2.35 / +0.76 s** late (0–0.5 / 0.5–1 / 1–3 Hz), the detector's zero-phase trigger fires **−0.88 s** early, total correction **1.64–3.23 s**. The draft's "+2.4→+4.9 s" peaks correct to roughly **+1 to +2 s** (corrected half-rise 1.2–2.7 s). Direction is unaffected — both biases push the measured event *later*, never earlier — so "no precursor" was never at risk from this.

  **(4) Filter startup bug in the causal estimator.** `sosfilt` began from zero state against a ~2000 fF DC level, so the envelope rang for minutes at these corners (settling ~1/0.03 Hz for the slow band): early windows were garbage and the NREM SD used for z-scoring was inflated. Fixed with steady-state initialisation + a 120 s warm-up exclusion (S2N2 CH first-60 s max 8.7 vs night median 1.84). **`lowband_precursor_check.py` still has it**, and that script produced manuscript Figure 9.

  **(5) Two metric artifacts that inflated the old consistency claim.** The "6/6 subjects" in `response_consistency_q30.csv` came from a per-subject post-onset **max**, which is max-of-noise biased upward and worst for low-n subjects — it inflated the null as much as the signal (CH 1–3 Hz null "peak" 1.59 z). On a 0–10 s window mean the counts are 5–6/6 against a matched null, and 0–3/5 once motion is controlled. Separately, raw z is the wrong scale for cross-subject amplitude: these envelopes are heavy-tailed (S2N2 CH night max **5454** vs median **1.84**), so SD is set by movement artifacts and varies several-fold between nights — the quietest night (S5, 6 onsets) read **5–13 z** where everyone else read 1–3, dominating both the cohort mean and the age analysis. Cross-subject amplitudes now use median/MAD.

  **(6) Selection-matched null — checked, and it was not the explanation.** Real onsets pass three gates (NREM, motion-clean pre-window, quiescent EEG-delta pre-window); the old null imposed only two, so the null baseline was drawn from more delta-rich epochs and could have manufactured the small pre-onset dip. Adding the quiescence gate to the null makes the dip slightly *larger* (−0.007→−0.171 z, 1–3 of 6 subjects positive), so it is not a null-selection artifact. **No precursor in any band still stands**, and the zero-phase estimator still shows a spurious positive slow-band lead (+0.10 to +0.21 z) where the causal one shows none — the original acausal-leak finding reproduces.
...
- **2026-08-06** — **Audit of the spindle result (§4.4 + Figure 7) for wrong interpretation and filter/measurement artifacts.** No new analysis scripts; re-derived from `analysis/spindles/outputs/{spindle_lowband_detection.npz,spindle_ersp.npz,spindle_ersp_control.csv}` plus fresh PSD/phase checks on the raw channels.
  ...
  **(1) The sigma-band null sits inside the paper's own electronic noise floor — the mechanistic conclusion does not follow.** §Methods and Figure 2 define the CAP noise floor as *"10 Hz to the Nyquist frequency, 50 Hz"* and state a no-subject baseline confirms it is white across 0.1–50 Hz. The spindle band (11–16 Hz) is entirely inside that. Measured directly: CAP PSD at 11–16 Hz is **1.76–2.01× the 40–49 Hz floor in all 12 sessions × 3 channels**; the highest frequency at which any CAP channel still exceeds 3× its floor is **0.0–6.9 Hz (median ≈1.5 Hz)**. EEG for comparison: corner **35.4 Hz**, sigma SNR **460–1200×**. So the mask carries no measurable content of *any* kind at 11–16 Hz, and the null cannot separate "does not transduce cortical electrical fields" from "noise floor exceeds any signal present". Reframe as a bound: pooled 14,304 N2 spindles give **|sigma effect| < 0.05–0.06 dB (<1.5 % power)** vs EEG **+3.32 dB (+115 %)**. Note CRE's 95 % CI is **[+0.005, +0.064] dB** — nominally excludes zero, so "null" is loose; the bound framing is both safer and stronger.

  **(2) The +0.5 dB low-band (0–3 Hz) effect is tail-driven, not a per-spindle response.** Pooled CH: mean **+0.58 dB** but median **+0.14 dB** and 10 %-trimmed mean **+0.24 dB**; the **top 5 % of spindles contribute 121 % of the mean** (CLE−CRE 126 %) — the other 95 % sum negative. Per-spindle detection rate (core > own baseline) is **0.521** (CH) against a matched-control null of **0.485**, and **3/12 sessions fall below 0.50** (S1N1 0.493, S4N1 0.495, S5N2 0.480). The group-level effect is real; "power rises at the spindle center … reproducible across all twelve nights" overstates what a single spindle does.

  **(3) Time-axis bug: everything labelled t = 0 is actually −0.32 s.** `spindle_ersp.py:71` and `spindle_lowband_detection.py:111` both use `tcen = t - t[-1]/2.0`; for a 1601-sample window the true centre is 8.00 s but `t[-1]/2 = 7.68 s`. Consequences: the "core" `|t| < 1 s` really spans **−1.28 to +0.64 s** (weighted *before* the spindle), and the baseline is asymmetric — pre −7.36…−5.44 s (1.92 s) vs post +4.80…+7.36 s (2.56 s). Fix is `tcen = t - WIN_HALF`.

  **(4) Annotations are almost certainly auto-detected, not "technologist-scored".** Every duration is an exact multiple of **250 ms** (0.50 s 36–66 %, 0.75 s 19–29 %, then 1.00/1.25/1.50); file header is `Signal ID: Spindel\spindel`, `Signal Type: Impuls`, under `PSG_analysis_*`. Event centres therefore carry **±125 ms** quantisation. Also, the per-event *frequency* file covers only **27–96 %** of events (S2N1 28 %, S2N2 27 %), and **S4N2 has no `Spindle  K` file at all** — the loader falls back to the frequency file, giving 554 events where comparable nights have 1800–2000. None of this is disclosed.

  **(5) The draft's alignment check is invalid; the manuscript currently states none.** `spindle_section_draft.md` offers "53–68 % of spindle centres fell within PSG N2 epochs" as evidence of correct alignment, but that is *at or below the N2 base rate* (N2 = 36–86 % of epochs; S1N1 52.7 % vs 49.5 % base, S6N1 73.9 % vs 76.9 % — below). The valid checks are REM/Wake exclusion (**0.0 % of spindles in REM; 0.0–1.6 % in Wake** vs up to 10.9 % Wake base) and the EEG +3.3 dB itself. Do not reinstate the N2-fraction sentence.
...
- **2026-08-05** — **Mean-centred mean value, head-angle revalidation, night-scale flow marker, CH vs CLE−CRE, and CAP variance vs cortical activity** (`analysis/mean_value/{mean_centred_traces,head_angle_validate,flow_marker,ch_vs_clecre_sessions}.py`, `analysis/swa_validation/cap_variance_vs_cortical.py`; figures under `writeup/figures/{mean_value,head_angle,flow,channel_evolution,cap_variance}/`). Five requests, all framed as "this is what the CAP can do".
  ...
  **(1) Mean plots referenced to zero, not z-scored.** Every mean-value trace is now `x(t) − mu` in femtofarads with `mu` (the sleep-period session mean) printed on the panel and tabulated. Operating points: CLE **1999 fF** [1958, 2048], CRE **2184 fF** [1624, 2353], CH **−961 fF** [−1297, −719], CLE−CRE **−185 fF** [−374, +356] — S5N1 is the only session with a positive L−R offset. Typical excursion about the mean is small: sd of CLE−CRE **21.5 fF** median (range 5.6–199). Per-stage deviations from the session mean are single-digit fF and do not separate stages: CLE−CRE Wake **+0.0**, N1 **+0.2**, N2 **+2.4**, N3 **+3.6**, REM **−1.2 fF**; CH is uniformly offset **+9 to +13 fF** across all stages. The N3-highest ordering is consistent with the earlier z-scored result but the effect is now visible for what it is — a few fF on a ~2000 fF baseline.

  **(2) Head angle revalidated — valid, but two bugs found.** (a) The accelerometer is in **g** and the DC vector really is gravity: |a| median **1.033 g** [1.014, 1.036], CV **1.0%** (max 6.8%, S4N1), 97.6–99.9% of each night within 5% of the session median → arctangent tilt angles are physically meaningful. (b) **The gravity low-pass was wrong**: 0.5 Hz is the top of the respiratory band, so breathing modulated the "static" angle. On motion-quiet epochs the breathing-band ripple in the turn angle is **0.186° at 0.5 Hz vs 0.008° at 0.05 Hz — 25x**. Default moved to 0.05 Hz. (c) **`classify_position()` had left/right inverted.** Positive roll (gY > 0) is the subject's LEFT, verified in **100% of lateral epochs across 8 sessions** against the PSG position channel. (d) The legacy clamped roll `atan2(gY, sqrt(gX²+gZ²))` folds: up to **25.6%** of a night is spent with gZ < 0 (head past the side), where median |legacy − true| is **67.9°** (vs 0.21° while gZ > 0). Replaced by `turn_deg = atan2(gY, gZ)`. (e) **Validation against the independent PSG-scored body position: 99.0% median agreement** [75.2, 99.9] over **9,319 scored epochs**, fixed thresholds, no per-session tuning. Per-position turn angle: Supine **−6.8°**, Left **+69.2°**, Right **−89.1°**, Upright elevation **+66.2°**. Confusion is one-sided and interpretable: PSG-Left → 461/463 correct, PSG-Right → 589/605, and essentially all disagreement is PSG-Supine → accelerometer Left/Right (482 epochs) — the PSG sensor scores TRUNK position, ours is on the head, so head-turns-on-pillow with the trunk supine are a real difference, not an error. (f) Ellipsoid calibration moves the angle by only **0.52°** (median) and is poorly constrained anyway (sphere coverage 0.21–0.66 on supine-dominant nights), so it is reported, not applied — the angles are ratios of axes, so isotropic gain cancels exactly.

  **(3) Flow marker: level, not velocity.** The old quantity was a differentiator (`drift_rate` = within-segment slope), which turns every electrode re-seat into a spike and averages to zero over the night by construction. Replaced with **magnitude** `A(t) = LP_tau(|d|)` (low pass on the absolute) and **direction** `D(t) = LP_tau(d)/A(t) ∈ [−1,+1]`, `d` = mean-centred CLE−CRE, low pass = 2.5 min median (de-spike) then 30 min rolling mean, motion epochs excluded. **Night-scale persistence (autocorrelation at a 30 min lag): velocity −0.005 [−0.032, +0.019] vs direction +0.518 [−0.008, +0.904]** (raw `d` +0.238). tau swept 5–60 min (persistence 0.25 → 0.78, reversals 9 → 2); 30 min chosen. Flow magnitude **12.3 fF** median across sessions (3.4–182), **4** direction reversals per night median. *Implementation note:* a rolling median at the tau scale was tried first and rejected — over 30 min `d` is nearly always one-signed so `median(d) == ±median(|d|)` exactly and `D` collapsed to a hard ±1. **Posture control:** rho(D, sin head-turn) median **−0.08** [−0.44, +0.65], significant in 11/12 sessions but with **mixed signs (5 positive, 6 negative)** — no consistent posture effect across subjects. Supine-only: magnitude **12.2 vs 12.3 fF**, reversals **3.5 vs 4.0** — the marker is not just the head turning.

  **(4) CH vs CLE−CRE over the sessions — not the same signal.** If CH were the arithmetic difference the offset would be 0 fF and the slope 1. Measured across 12 sessions: offset median **−742 fF** [−1075, −636]; slope median **1.57** [−1.86, +6.45], **negative in 3/12 sessions**; CH moves **2.8x** as far as CLE−CRE (sd ratio, range 1.6–11.0); r(level) median **0.55** [−0.49, +0.96] (3/12 negative), r(1-min change) median **0.59** [−0.65, +0.99]. **Agreement tracks signal size: Spearman rho = 0.73 (p = 0.007)** between the sd of CLE−CRE and their change-correlation — the two channels agree on the nights the differential actually moves and decouple on quiet nights. Band-resolved coherence stays graded slow **0.55** > respiratory **0.31** > cardiac **0.09** (reproduces the 2026-07-30 result).

  **(5) CAP variance vs cortical activity (prof's hypothesis).** Split into two claims. **Claim 1 — CAP variance tracks EEG band power:** the association is **specific to EEG beta and to nothing else**. CAP total variance vs EEG beta rho **+0.45 → +0.41 after partialling out head motion, same sign in all 6 subjects**; CAP resp/beta +0.34, card/beta +0.26, theta/beta +0.12. Every other EEG band is flat — median |rho| across all 45 band pairs is **0.03**, and only **4/45 pairs** keep their sign in every subject, all in the beta column. Motion is not the explanation. **Caveat recorded in-file:** scalp beta is the EEG band most contaminated by EMG and both signals sit over temporalis, so a shared muscle-tone term is a live alternative and is NOT excluded. **Claim 2 — arousals the EEG misses:** the intended test was not available. The PSG's two arousal scorings are **nearly the same set — 98.9% (median) of autonomic (pleth) arousals are also EEG-scored, leaving only 42 autonomic-only epochs in the entire cohort** (0–7 per night out of ~800), so there is no "body aroused, EEG saw nothing" population. Tested against the EEG *readout* instead: single-feature AUC for detecting a scored arousal is **CAP variance 0.608** [0.490, 0.720] (11/12 sessions > 0.55), **EEG beta 0.592** (8/12), **head motion 0.566** (7/12). Restricting to scored arousals an EEG power detector would miss (EEG beta below the session median) vs matched non-arousal epochs in the same low-EEG regime, n = **1,564** epochs: CAP variance still higher in **11/12 sessions** (median **1.04x**, 8/12 with p < 0.05, AUC **0.574**), while motion is higher in only 6/12. Peri-event: 1,529 cortical-arousal onsets show a **1.06x** variance rise locked to onset. **Effect sizes are small and reported as consistent-across-subjects rather than large.**

- **2026-07-30** — **Capacitance units, CH-vs-CLE−CRE, and intracranial pressure across sleep** (`analysis/mean_value/{ch_vs_diff,icp_across_sleep}.py`; `sleep_monitor/config.py`; figs `writeup/figures/channel_evolution/{ch_vs_diff_summary,icp_across_sleep}.png`). **Request (user):** hypnogram row as a depth ladder; mean value in femtofarads ("true capacitance from the FTD001 sensor"); "CH and CLE−CRE should mean the same thing — analyse how they differ"; and quantify the overall mean change as intracranial fluid pressure across sleep. **(1) Units.** CAP columns declared as femtofarads via a single `CAP_SCALE_TO_FF = 1.0` in config. Evidence: on-head values ~2000 (≈2 pF, right order — source paper reports bare-sensor C0 = 739.7 ± 20.9 fF), and the no-subject baseline file `baseline noise/SM2_33.txt` carries the same channels at exactly 1/1000 the magnitude (so that file is pF). **Caveats recorded in-file:** neither source paper states the recorded files' units (bench values fF, nap-study Methods "a.u."), and **"FTD001" appears in neither paper — both name the TI FDC1004** ("sub-femtofarad resolution", 2.4 V @ 25 kHz). **(2) CH is NOT CLE−CRE (n=12).** The 2025 paper does call CH the interhemispheric L−R difference (p.5, positive = higher left r-ICP) but never writes CH = CLE−CRE, and describes four sensors → three signals. Measured: offset median **−764 fF** (range −1075 to −636; would be 0), OLS slope median **1.58** (range −1.82 to +8.73; would be 1), r(level) median 0.55 (range −0.48 to +0.96). Coherence is **band-graded — slow 0.55 > respiratory 0.31 > cardiac 0.09** — i.e. the two channels share the slow common-mode drift but are essentially independent at cardiac frequencies. Agreement scales with the size of CH's own excursion (6 sessions with resp-coherence ≥0.36 have median sd_CH 210 fF and r 0.86; the other 6 have sd_CH 37 fF and r 0.06). **It is NOT motion artifact** — r on the motion-quiet half is *higher* (0.64) than on all epochs (0.55). Conclusion: the two are not interchangeable; `mean_CH` was also missing from `mean_value_epochs.csv` entirely and has been added. **(3) Pressure across sleep — mostly NEGATIVE, one survivor.** Referenced each epoch to its session's Wake median, in fF, with mmHg only as a flagged secondary axis (`ICP_SENS_FF_PER_MMHG = 4.79`, the sole sensitivity in the source work: porcine, CLE, respiratory band, linear only to 8.2 mmHg). **Raw (drift kept):** stage effects look sizeable (CLE N3−Wake +5.2 fF, CLE−CRE +6.9 fF) but **no channel reaches KW p<0.05 (best p=0.21)** and per-subject direction is 3+/3− or 4+/2−. **The reason is a confound worth recording:** the overnight swing (median **58.5 fF**, range 30.7–155.7) is ~10× the stage effect, so a single drift-dominated night inverts a subject — CLE N3−Wake is **9/12 nights positive** yet only 3/6 subjects positive, because S1N2 (−18.1) and S3N2 (−39.4) are drift-scale, not stage-scale. **Detrended (61-epoch ≈30 min centred rolling median removed, fF preserved):** all stage effects collapse to ≤1 fF, and one result survives properly — **CRE (right) REM > Wake, +0.6 fF (+0.12 mmHg), 6/6 subjects, 10/11 nights, KW p=0.0053**; CLE N3>Wake is 5/6 subjects but only 7/12 nights (weak). Consistent with the 2025 paper's "REM was identified by relatively high magnitude-ΔC". **Also flagged:** the median overnight swing converts to 12.2 mmHg, which *exceeds* the 0–8.2 mmHg range over which the calibration was validated, so that conversion is extrapolation and the swing is more plausibly coupling/posture drift than true pressure. **Verdict:** no robust stage-locked intracranial pressure change in this cohort; the mean is dominated by slow drift, and the only reproducible stage signal is a small right-side REM elevation.
- **2026-07-23** — **Per-session, per-channel overnight-evolution reporting figures** iterated to final form (`analysis/mean_value/channel_evolution.py`; 48 figs `writeup/figures/channel_evolution/<SESSION>_<CLE|CRE|CH|CLE-CRE>.png`). **Request (user, multi-turn):** journal-style per-channel panels of mean/variance/velocity/motion + spectrogram; channels CLE, CRE, CH; spectrogram 0–5 Hz; 10 Hz-capped mean/variance; then "regress motion out, keep velocity continuous (don't cut)"; then "capture the SUPER-slow rate of change of the mean, velocity is dominated by oscillations"; then "add CLE−CRE as a flow channel (CSF flow across brain; balanced = good sleep)". **Design (6 rows A–F):** hypnogram / mean (adaptive median±5·MAD y-axis so huge coupling swings clip and the baseline structure stays visible) / variance (10 Hz-capped so it's physiological-band power, not the >10 Hz electronic floor) / **super-slow drift rate** / motion / 0–5 Hz spectrogram with Resp+Cardiac band lines + dB colorbar. **Two methodological findings that mattered:** (1) **Velocity = local linear slope over a 25-min window (Savitzky-Golay 1st-derivative) of the accelerometer-regressed baseline, NOT low-pass-then-differentiate.** Differentiating a low-passed signal always re-injects oscillation (derivative amplifies ∝ frequency), so 0.15/0.05/0.01 Hz caps all stayed oscillation-dominated; the windowed slope averages out anything faster than the window and yields the true drift trend — near-zero over flat-baseline stretches, sustained signed value where the mean ramps (verified against row B on S1N1 clean + S6 poor-contact). (2) **Motion is regressed out (OLS on per-axis accel MEAN = gravity/head-orientation + movement magnitude), not blanked** — continuous velocity, and electrode coupling-loss STEPS (poor-contact S6) correctly remain since they aren't accelerometer motion. **CSF-flow channel (CLE−CRE):** row B rendered as directional flow L−R with two-colour fill about the neutral (session median — offset-invariant, because the absolute L−R level is dominated by a static mount offset per [[project_flow_imbalance]]); balance readout = # side-to-side reversals + longest one-sided run (min). Balanced flow = frequent reversals/short runs (hypothesised good sleep). **Observation:** flow drifts side-to-side across the night (S1N1: 117 reversals, longest run 66 min, shifts R-dominant→L-dominant); the balance metric is descriptive-only for now — the offset-invariant balance-vs-sleep-quality correlation across 12 sessions is the pending statistical test (groundwork in `flow_imbalance_vs_quality.py`). **Status:** reporting figures for the paper; representative-session main figure (3–4 channel, vector PDF, shared dB scale) + supplement set still pending user's paper-level packaging decision.
- **2026-07-23** — **Ridge + harmonic analysis, revamped** (`analysis/slow_wave/ridge_harmonic_revamp.py`; module upgrades in `sleep_monitor/harmonics.py`; outputs `reports/slow_wave/revamp/*`; figs `writeup/figures/harmonics/revamp_{overlay_S1N1,by_stage,continuity,harmonics}.png`). **Request (user):** "the ridge and harmonic analysis in paper is stale, revamp it" → redo the analysis, make long-lasting ridges flat (merge the bits-and-pieces), make harmonic detection strong, and add a slow band (~0.1 Hz). **Method / three upgrades:** (1) **Flat long ridges** — added `fill_gaps` (linear-interp each ridge's freq/amp over internal holes) + iterative-to-convergence fragment merging on boundary-median frequency to `detect_persistent_ridges` (backward-compatible; jitter/flatness still measured on the untouched raw trace, `coverage` still reports true pre-fill detection density). (2) **Slow band 0.02–0.15 Hz** detected with long 240 s windows / 120 s Welch segments (df≈0.008 Hz) — the infra-slow/vasomotor band that the CAP-SWA slow-DC-drift marker lives in. (3) **Strong harmonics** — integer-ratio ladders validated against a per-k surrogate null (random ridge freqs over the observed band), keep only ≥3-member ladders beating the null 95th pct, with a calibrated confidence (CH channel). Ran 3 bands × 3 channels × 12 sessions, stage stats pooled on CRE. **KEY FINDINGS:** (a) **Continuity fixed:** pooled CRE, mean trace coverage 0.88–0.94 → **1.00** (internal holes ~3000 → **0**), fragments merged (resp 35→22, slow 21→15 ridges/session); longest continuous flat ridge now **150 min (resp), 124 min (slow)** — the "bits and pieces" are gone. (b) **NEW slow-band result:** a persistent slow ridge is present in **88–92%** of clean epochs on all three channels, median lowest freq **~0.07 Hz (≈14 s period)** — a near-ubiquitous infra-slow oscillation the mask carries. Its **total ridge power drops sharply in N3 in all 6 subjects** (N3 median 0.083 vs 0.369 elsewhere; KW p=9×10⁻⁵³; N3-vs-rest MWU p=6×10⁻⁵²; pooled N3 AUC≈**0.68** in the low-power direction) — a **new cross-subject-consistent N3 marker in the mechanical SWA band**, matching the CAP-SWA score (0.68) and the slow-DC-drift story [[project_slow_wave_analysis]]; slow-band lowest freq is slightly *higher* in N3 (weak, 4/6). (c) **Resp band** reproduces the prior story on the flat ridges: fewer active respiratory ridges in N3 (6/6), lower total power (5/6), lowest freq ≈0.23 Hz (=resp rate). (d) **Cardiac band:** lowest ridge freq lower in N3 (median 1.0 vs 1.125 Hz, 5/6) — the deep-NREM bradycardia. (e) **Harmonics now rigorous:** 6,812 strong (≥3-member) ladders on CH, **6,783 survive the surrogate null = 99.6% real** (per-session 98.9–100%), median confidence **0.75**, median fundamental **0.25 Hz** (respiratory), median **4 members** — the ladders are integer harmonics of the respiratory fundamental extending into the cardiac band, essentially never coincidental. **Takeaway:** the revamp turns the fragmented ridge picture into continuous flat multi-hour ridges, adds a genuinely new and 6/6-consistent slow-band N3 marker (AUC≈0.68) that ties the spectral ridge story to the mechanical CAP-SWA story, and puts the harmonic-ladder claim on a firm near-zero-false-positive footing. Supersedes the 2026-07-16 band-restricted analysis for the manuscript §3.6/§4.3. Draft replacement text pending (docx locked open in Word at write time).
- **2026-07-23** — Delta-onset CAP-precursor test (`analysis/delta_onset/delta_cap_precursor.py`) — the actual test of the professor's hypothesis that a CAP band event (0–0.5 / 0.5–1 / 1–3 Hz; CLE/CRE/CH) *precedes* EEG delta-burst onset. **Method:** consumes the q15/q30 delta-onset triggers; per band×channel runs three tests each with a null — (1) peri-onset average of z-scored CAP band power locked to onset vs a random-NREM null (aggregated per-subject, n=6), (2) CAP→EEG-delta cross-correlation over contiguous NREM (+lag = CAP leads; phase-ramp circular-shift null), (3) forecasting AUC of pre-onset CAP power ([−12,−2] s) vs random NREM. 0–0.5 band high-passed 0.03 Hz to kill DC/coupling drift; features decimated to 20 Hz + cached; xcorr computed once/session. **Result — professor's hypothesis NOT supported; direction is REVERSED:** no CAP precursor before onset (lead-window amplitude −0.02→−0.11 z, at/below baseline; forecasting AUC 0.42–0.56 ≈ chance, per-subject dots straddle 0.5). **Instead a strong CAP band-power RESPONSE after onset** — all 3 bands × 3 channels rise sharply at t=0, peak +2→+5 s, far above the flat random-NREM null (CH>CRE>CLE). Cross-correlation peaks at lag ≈0 with the shoulder on the EEG-leads (negative-lag) side → CAP *follows* delta, doesn't lead. Robust to window (q15≡q30), consistent across subjects. Minor nuance: the slow 0–0.5 band starts creeping up a few s before the higher bands (CRE/CH) but stays within the null in the lead window — likely onset-timing jitter of the broad slow envelope, not a real precursor. **Interpretation:** the mask carries a genuine slow-wave/K-complex-locked mechanical–hemodynamic co-activation (consistent with the spindle-ERSP 0–3 Hz bump [[project_spindle_validation]] and CAP-SWA [[project_slow_wave_analysis]]) but it is a *following/concurrent* event, not a preceding one. Figures `analysis/delta_onset/outputs/fig_precursor_{grid,xcorr,auc}_{q15,q30}.png`; tables `precursor_summary_{q15,q30}.csv`. For the professor: the preceding-event hypothesis is negative, but there is a robust *post*-onset CAP correlate worth framing.
- **2026-07-23** — Corrected the manuscript in-band SNR (Figure 2) from band-integrated to **per-Hz spectral density**, after the baseline-noise recording + a raw-waveform sanity check exposed the old metric as mislabeling good channels as failed (`writeup/figures/signal_validation/inband_snr.py`; `fig2_inband_snr.png`; `inband_snr_summary.csv`). **Request (user):** a no-subject baseline noise recording was added (`baseline noise/SM2_33.txt`, ~16 min, CH/CLE/CRE @ 111 Hz) — use measured noise for the SNR instead of the frequency-based floor; then, on inspecting per-session PSDs, "why do we have bad data in some session, increase signal band to 30 Hz?"; then, from raw data, "it doesn't look failed in any session." **Investigation:** (1) Baseline PSD is spectrally **white 0.1–50 Hz** (HF/in-band density ratio ≈1.0–1.06, no 1/f rise) — this *validates* using the ≥10 Hz floor as the in-band noise estimate, but the baseline is on a different unit scale than the overnight recordings (raw ~1× vs ~1000×; power floors differ ~10¹²), so it can NOT be plugged into the SNR denominator directly — its role is validation, not substitution. (2) The old metric integrated signal over 2.9 Hz (0.1–3) vs noise over 40 Hz (10–50) — a fixed ~11 dB bandwidth penalty. Decomposition showed "bad" sessions were **low signal, not high noise** (floor near-constant 0.016–0.033 across all 12); I initially over-called these "failed/dead electrodes." (3) **User was right:** loading the raw 0.1–3 Hz bandpassed waveforms, S5N1 CRE (the −5.1 dB "worst") has std 0.27 a.u. vs 0.34 for a good night, and S1N2 CRE std 0.62 — clear rhythmic resp/cardiac in every "bad" case. Re-scored as per-Hz **density**, the worst is S5N1 CRE at +6.4 dB (4.4× floor); the rest 12–13 dB. No failures. (4) Extending signal band to 30 Hz REJECTED: baseline proves 3–30 Hz is pure white noise, so widening the band raises the weakest bars the most (S5N1 CRE −5.1→+7.3) — it would fold noise into "signal" and disguise the low end. **Fix applied:** `snr_db` now uses `band_density` (mean PSD/Hz) for both bands; equal-bandwidth + gain-invariant (S4N1 gain anomaly cancels in the ratio). **Result:** channel means CH 18.2→30.0, CLE 7.1→18.7, CRE 9.0→20.7 dB; nothing below +6 dB; ranking preserved (CH strongest every night, temple electrodes modest but usable). No "bad-data session" — the earlier framing was a metric artifact. **Takeaway:** the numbers now track the visible raw waveforms; caption cites the white baseline floor as justification. Baseline diagnostic figures (raw traces, per-channel spectrum, log-log PSD, good-vs-bad grid) live in scratchpad — not yet saved to repo pending user decision.
- **2026-07-23** — Delta-onset event detection — building the "trial split" for the professor's hypothesis that at EEG delta-burst onset a *preceding* event appears in CAP bands 0–0.5 / 0.5–1 / 1–3 Hz of CLE/CRE/CH (new workspace `analysis/delta_onset/`, `delta_onset_detection.py`). **Request (user):** build the onset detector first so we can eyeball events before committing to the peri-event/lead-lag analysis. **Method:** Schmitt-trigger burst detector on the EEG delta (0.5–4 Hz) Hilbert envelope; per-session robust baseline over NREM (median + MAD), high=med+2·MAD, low=med+0.5·MAD; burst must stay above `high` ≥4 s; **onset = walk back to the rising edge (last low-crossing)** so t=0 is the burst rise, not its peak; refractory ≥25 s; kept only if onset in NREM (N2/N3) and the −30→0 s pre-window is inside the recording with motion fraction <0.10 (motion = rolling-std of accel > 90th pct). Outputs per-session onset npz + `delta_onsets_summary.csv` + overview (hypnogram/envelope/motion) and single-event gallery figs. **Findings:** 11–197 onsets/session (S5N1 low at 11 — short 2.5 h NREM night; S2N1 197), 100% in NREM by construction, **N2-dominant** (K-complexes/isolated slow waves outnumber N3 SWA), median IEI 55–290 s. Galleries confirm clean quiet→burst transitions with clear slow-wave/K-complex morphology in the raw EEG for the majority of events. **Open issue surfaced by eyeballing:** low-SWA sessions (S5N1) and a subset of panels show pre-window delta already elevated (z-env wanders ±2 before t=0) → a CAP "precursor" there could just track pre-existing EEG delta. Decision pending before the precursor step: add a pre-window EEG-quiescence gate (cleaner trials, fewer n) vs. keep permissive and regress out pre-window delta downstream. Next: `delta_cap_precursor.py` (3×3 peri-onset band-power grid, CAP→EEG xcorr testing negative lag, forecasting AUC, with random-NREM + shuffle nulls). **Update (same day):** per user decision, added `REQUIRE_QUIET_PRE` EEG-quiescence gate (mean pre-window delta env < `low`). Galleries now show uniformly flat pre-windows → clean quiet→delta transitions. **New finding / tradeoff:** the strict full-30 s gate drops counts sharply (S2N1 197→99, S2N2 181→80) and **removes N3 almost entirely (0–1/session)** — principled, since N3's sustained delta has no quiet baseline for an "onset" to rise from, so the gate selects isolated N2 slow-wave/K-complex onsets. Downside: S5N1=1, S4N2=4, S5N2=6, S1N2=9 are now too thin for per-subject event-triggered stats. Open decision: shorten the quiescence window to the immediate pre-onset (~10–15 s) to recover n while keeping the transition clean, vs. keep strict and report only the ~6–8 well-populated (S1N1, S2N1/2, S3N1/2, S6N1/2) sessions. **Resolved (user):** keep BOTH 15 s and 30 s as parameters and report both — detector now sweeps `--quiet-pre` windows, tags outputs `q15`/`q30`, and writes a side-by-side count pivot. **Result:** 15 s recovers only modest n over 30 s (S2N1 99→108, S6N2 17→28, S4N2 4→9) and does NOT rescue the genuinely thin nights (S5N1=2, S5N2=7) — subject 5's shortfall is a real low-SWA/noisy-EEG data property, so S5 likely drops from per-subject (n=6→~5) stats regardless of window. Galleries at both windows show clean flat pre-windows. Proceeding to `delta_cap_precursor.py` will run on both onset sets (q15/q30) as a robustness check.
- **2026-07-21** — Per-session, per-channel overnight-evolution journal figures for the paper (`analysis/mean_value/channel_evolution.py`; 36 figs `writeup/figures/channel_evolution/<SESSION>_<CLE|CRE|CH>.png`). **Request (user):** for each session, one figure per channel (CLE, CRE, CH) showing mean value, variance, smoothed velocity, motion, and a spectrogram, with y-axes autoscaled so the evolution is visible; spectrogram 0–5 Hz; low-pass mean & variance at 10 Hz; journal reporting style. **Design:** 6 stacked rows A–F (hypnogram strip / mean / variance / baseline velocity / motion / 0–5 Hz spectrogram + dB colorbar), raw channel LP-filtered <10 Hz before mean/variance (so variance = physiological-band power, not the >10 Hz electronic floor), 10 s blocks, robust 1–99th-pct per-row autoscale, faint per-stage shading. **Observations across the set:** mean value shows the coupling-offset baseline drifting/stepping over the night (good sessions span tens of a.u.; the poor-contact subject **S6** shows coupling-loss excursions of ~1000–1800 a.u. with step recoveries — matches [[project_cap_mean_drift]]); variance spikes align with the motion row (movement artifacts) and go quiet in consolidated N2/N3; spectrogram resolves the respiratory ridge (~0.25 Hz) and a cardiac ridge (~1 Hz) on all channels, CH strongest. Figures are the visual companion to the absolute-mean-value analysis (same-day entry below). Awaiting user review before any go into the manuscript.
- **2026-07-21** — Absolute (native a.u.) CAP mean value vs sleep stage — the "not just z-score" analysis the professor asked for as the Results opener (`analysis/mean_value/abs_mean_vs_stage.py`; figs `notebooks/plots/mean_value/abs_{scale_comparison, stage_boxplot_au, baseline_by_session, trace_S1N1/S3N1/S5N1}.png`; tables `reports/mean_value/abs_mean_{scale, stage_au, subject_direction}.csv`; draft `writeup/edits/abs_mean_value_section_draft.md`). **Question (user):** add to the paper an analysis on the direct absolute mean value of the CAP sensors, not just the z-score. Consumes the raw a.u. columns already in `mean_value_epochs.csv`; controls the between-session DC offset by *subtracting* per-session Wake median (preserves a.u. scale, unlike z-score which also divides by SD). **KEY FINDINGS (honest characterization — absolute level is coupling-dominated, not a usable stage marker):** (1) **Offset-dominated:** per-session baseline SD across the 12 nights = 30 a.u. (CLE), 175 (CRE), 170 (CLE−CRE), 93 (CLE+CRE) — set by that night's mask/sensor–skin coupling. (2) **Drift-confounded:** monotonic within-night baseline drift |median| 4–25 a.u. (unrelated to stage), comparable to any stage effect. (3) **Stage excursion is small and not universal:** within-session centered N3−Wake median = **+7.0 a.u. CLE, +4.6 CLE−CRE** (N3-elevated), but only +0.7 CRE and split on common mode; N3-elevated direction in **5/6 subjects** for CLE and CLE−CRE, 2/6 CRE, 3/3 CLE+CRE. Variance ranking: coupling offset ≫ night drift ≳ stage. (4) Pooled Kruskal–Wallis is p<10⁻⁷ every channel but that's epoch-level pseudoreplication (n=6 subjects) — demoted to a parenthetical, per-subject direction reported instead (same fix as HL#51 / SUBMISSION_REVIEW P1#7). On the raw a.u. axis the channel baselines are near-flat all night; the stage-linked excursion is <~0.3% of the ~2000 a.u. baseline (fig `abs_trace_*`). **Takeaway:** the absolute mean value is a coupling-offset + drift-dominated baseline with only a few-a.u. N3-elevated excursion on the left/temple-differential channels (5/6 subjects, not channel-universal) — it earns a place as the Results opener because it answers the professor's directive AND justifies the per-session normalization used everywhere else; parallels the subject-dependent HER direction. Draft subsection + a.u. table ready in `writeup/edits/abs_mean_value_section_draft.md`; replaces the leftover HL#6 outline note.
- **2026-07-16** — Band-restricted (resp/cardiac) ridge + harmonic detection for the paper, replacing the broadband harmonic/ridge story (`analysis/slow_wave/band_ridge_analysis.py` computes, `band_ridge_figure.py` makes the paper figure; fig `writeup/figures/harmonics/band_ridge_by_stage.png`; per-epoch table `reports/slow_wave/band_ridge_epochs.parquet` 55,914 rows; summary `reports/slow_wave/band_ridge_stage_summary.csv`). **Question (user):** trim the broadband ridge/harmonic detection and add resp- and cardiac-band-specific ridge/harmonic detection. Added a backward-compatible `min_freq` param to `detect_persistent_ridges` (in `sleep_monitor/harmonics.py`) so the spectral-peak search + ridge-frequency floor can be band-limited. Ran ridge detection twice per session×channel: **respiratory band 0.1–0.5 Hz** (single 30 s periodogram for frequency resolution, max_freq_jump 0.05) and **cardiac band 0.5–3.0 Hz** (8 s Welch segments, max_freq_jump 0.10); pooled stats on CRE; KW across stages + MWU N3-vs-rest + per-subject direction. **KEY FINDINGS (physiologically interpretable, and — unlike the old broadband HER — direction-consistent):** (1) **Respiratory band:** a persistent ridge is present in **96.7%** of clean epochs; its lowest frequency sits at median **0.23 Hz (≈14 br/min)** in every stage (it IS the respiratory rate). N3 has **fewer** active respiratory ridges (median direction N3-lower in **all 6 subjects**, MWU p=6×10⁻⁴) and lower total ridge power (KW p=3×10⁻²³) — the more regular, lower-effort breathing of deep sleep. (2) **Cardiac band:** persistent ridges are **intermittent (present in only 11.7%** of epochs), more frequent in N3/N2 than lighter stages; when present, the lowest cardiac ridge frequency is markedly **lower in N3 (median 0.88 Hz ≈52 min⁻¹) vs other sleep (1.25 Hz ≈75 min⁻¹), MWU p=2×10⁻⁸** (all 3 subjects with sufficient cardiac-ridge coverage) — the sleep bradycardia of deep NREM. (3) Harmonic *groups* within a single narrow band are rare (medians 0) — integer-ratio ladders don't fit inside one band — so the band story is carried by ridge count/power/frequency, not multi-rung ladders. **Takeaway:** band-restricting the ridge detector turns the messy, subject-dependent broadband harmonic result into two clean, consistent, mechanically-interpretable stage associations (respiration regularizes, heart rate slows in N3) — both autonomic/respiratory, NOT cortical, consistent with the paper's sensing thesis. These replace §4.4 (was Harmonic spectral structure + LOSO N3) in the manuscript.
- **2026-07-16** — Edited the canonical manuscript `writeup/main/CAP_sleep_mask_manuscript_main.docx` per user request to remove three negative-result threads and re-anchor the harmonic section on the band-restricted analysis above. **Removed:** §4.3 Within-session rate tracking (negative) + its Methods §3.6 + Discussion subsection + Figs 6–9 + Table 4; the LOSO N3 classification subsection of §4.4 + Fig 12 + Table 5 + its Methods sentence + Discussion subsection; §4.5 SEC-vs-contact-EEG SWA (negative) + its Methods §3.8 + Discussion subsection + Figs 13–14 + Table 6 (and the now-uncited Lucey ref #30). **Fixed** all cascading cross-references (spindle §4.6→§4.4 no longer cites §4.5; Discussion/Limitations/Conclusion rewrites drop tracking/SWA/LOSO). **Renumbered** Methods 3.6/3.7, Results 4.3/4.4, Figures 1–8, Discussion 5.1–5.5. **Rewrote** §4.3 "Harmonic structure and band-restricted ridge features" with the real band numbers and swapped the pooled-harmonic figure image for `band_ridge_by_stage.png`. Clean edit (no tracked changes; prior redline already accepted); XSD-validated (629→475 paragraphs). Pre-edit backup `CAP_sleep_mask_manuscript_main_PRE_NEGATIVE_REMOVAL_backup_20260716.docx`. **Then (per user "make a single final document"):** removed the internal "OPEN ITEMS / REVIEW NOTES" appendix (Conclusion → bibliography, 11 elements) so `CAP_sleep_mask_manuscript_main.docx` is the single clean final; references now end at #29; re-validated (475→464 paragraphs, PASSED). The three other `writeup/main/*_backup_*.docx` files are left untouched per user request.
...
## 2026-07-30 — Analytical review of the manuscript: three substantive findings

Reviewing `writeup/main/CAP_sleep_mask_manuscript_main.docx` against the artifacts turned
up three things that are analysis findings, not editorial ones.

**1. The respiratory spectral estimator is degenerate.** `sleep_monitor/rates.py:29` sets
`nperseg = max(64, fs*4)` = 400 samples = 4 s at fs = 100 Hz, so df = 0.25 Hz. The
respiratory band 0.1-0.5 Hz therefore holds two usable bins, and in
`artifacts/mask_phase_a.parquet` the resp `r_spectral` column takes exactly **two distinct
values (0.25 and 0.5 Hz), returning 0.25 Hz (= 15 br/min) in 99.95% of all epochs**. The
reported respiratory "MAE" of 0.91-0.95 br/min is therefore the error of a constant
predictor rescaled by a per-session k that is itself fitted to the reference. Non-degenerate
estimators match it (`fused_agree` 0.94 br/min, 5,324 distinct outputs), so nothing is lost
by switching, and the manuscript now reports the fused estimator with the degeneracy
disclosed.

**2. "Night-level MAE" was never a night-level quantity.** Everything the paper called
night-level accuracy is the *median of per-epoch absolute errors*. The genuine night-level
error, |mean(estimate) - mean(reference)| per recording, computed from
`artifacts/mask_phase_c.parquet`, is **0.14 br/min (IQR 0.10-0.41)** and **1.60 BPM (IQR
1.20-2.21)** - 6x and 2x smaller. S2N1 is the clearest case: a reported per-epoch error of
11.67 BPM against a night-mean error of 0.27 BPM, i.e. the per-epoch errors cancel almost
perfectly. Reporting both separately makes the aggregate claim stronger *and* correctly
labelled.

**3. Table 3's respiratory row was a chimera of two pipelines.** The median 0.91 and min
0.56 come from `symmetric_tracking_mae_table.csv` (resp/spectral/**CRE**, whose true IQR is
0.78-1.09 and max 1.34); the q75 1.19 and max 2.26 come from `per_session_summary.csv`
(**diff_spectral**). The lower quartile 0.81 appears in no file. Cardiac 3.41 [3.06-8.38] is
single-channel CRE unsmoothed while Table S1's per-session column is `fused_agree` +
smoothing (3.36 [2.64-6.62]) - two pipelines presented as one. Table 3 and Table S1 are now
generated from the same source.

Smaller corrections: analysis epochs are **30 s non-overlapping**, not "one-minute" or
"60-second windows with 30-second step" (S1N1: 954 epochs over 7.94 h, dt = 30.0 s), which
also explains Table 1's epoch counts; the k "IQR" [0.94-2.24] was the full range (true IQR
1.79-2.00); the age correlation rho = -0.83 has exact permutation p = **0.058**, not the
0.042 the t-approximation gives; and the error-to-variation ratios 0.77/0.78 were computed
from *different estimators* per band (resp from spectral, card from Detector B) and do not
distinguish the bands - that row was removed.

Also folded into the review copy: the SEC-vs-EEG SWA negative (r = -0.014 +/- 0.036, N3 AUC
0.490 +/- 0.041 vs 0.740 for the EEG pipeline itself) and the mechanical CAP-SWA/ISWA score
(per-subject LOSO AUC 0.675 +/- 0.073, 6/6 subjects above chance), which together finally
deliver the ISWA construct the Introduction had been promising and never defining.
