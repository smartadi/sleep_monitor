"""
Show the low-band (0.1-3 Hz) CAP spindle response ON DATA, trial by trial.
...
Figure (writeup/figures/spindles/fig_spindle_trials_lowband_<session>.png):
  (A) Center-triggered AVERAGE CH low-band power +/- SEM over all N2 spindles in
...
NOVERLAP = 96
SIGMA = (11.0, 16.0)
LOW_03 = (0.1, 3.0)   # excludes the f=0 bin; matches spindle_lowband_detection
CH_COLOR = '#2980B9'
EEG_COLOR = '#7F8C8D'
...
def main():
    ...
    # (A) center-triggered average across the whole top row
    ...
    axA.set_title(f'(A) {args.session}: CH low-band (0.1–3 Hz) power, center-triggered average '
                  f'over all {len(stack)} N2 spindles — peak +{avg[core_t].mean():.2f} dB at the spindle center  '
                  f'(per-trial detection rate {det_rate*100:.0f}%)',
                  fontsize=10.5, fontweight='bold')
    axA.set_xlabel('Time from spindle center (s)', fontsize=9)
    axA.set_ylabel('CH 0.1–3 Hz power\n(dB vs own baseline)', fontsize=9)
    ...
    for k, ei in enumerate(picks):
        ...
        if c == 0:
            ax.set_ylabel('CH 0.1–3 Hz (dB)', fontsize=7)
        if r == 3:
