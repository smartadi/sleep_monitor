"""
A night-scale capacitance-imbalance marker: low-pass the magnitude, then
read the direction.

What was wrong with the previous marker
---------------------------------------
The earlier imbalance quantities were differentiators. `drift_rate` in
imbalance_slow_decomp.py is the within-segment slope of the L-R differential, i.e. a
...
a direction, each low-passed to a night-scale timescale:
    ...
    A(t)  = LP_tau( |d(t)| )           MAGNITUDE — how one-sided the imbalance is
...
Outputs
-------
    reports/mean_value/imbalance_epochs.csv     A(t), D(t) per 30 s epoch
    reports/mean_value/imbalance_session.csv    per-session summary
    reports/mean_value/imbalance_tau_sweep.csv
    writeup/figures/imbalance/imbalance_definition.png
    writeup/figures/imbalance/imbalance_grid.png
    writeup/figures/imbalance/imbalance_summary.png
    writeup/figures/imbalance/imbalance_vs_headangle.png
...
Usage:
    .venv/Scripts/python.exe analysis/mean_value/imbalance_marker.py
"""
...
FIG_DIR = ROOT / 'writeup' / 'figures' / 'imbalance'
...
def imbalance_marker(d, motion, tau_min=TAU_MIN, despike_min=DESPIKE_MIN,
                     block_min=EPOCH_MIN):
    """
    Magnitude and direction of the differential imbalance.
...
def summarise(ep):
    ...
    for lbl, g in ep.groupby('session'):
        ...
        if len(g) < 40:
            continue
        D = g['imb_dir'].to_numpy()
        A = g['imb_mag_fF'].to_numpy()
        t = g['t_hr'].to_numpy()
        ...
        row = {
            ...
            'imb_dir_lag1_acf': acf_at(D, 1),
            'velocity_persist_acf': acf_at(v, LAG_EP),
            'imb_dir_persist_acf': acf_at(D, LAG_EP),
        ...
        if sup.sum() > 30:
            ...
            row['imb_dir_median_supine'] = float(np.nanmedian(D[sup]))
            row['imb_mag_median_supine_fF'] = float(np.nanmedian(A[sup]))
            row['imb_dir_reversals_supine'] = int(
                np.sum(np.diff(np.sign(D[sup][np.abs(D[sup]) > 0.2])) != 0)
                if (np.abs(D[sup]) > 0.2).sum() > 1 else 0)
        rows.append(row)

        for tau in TAU_SWEEP:
            Dt = g[f'imb_dir_tau{int(tau)}'].to_numpy()
            At = g[f'imb_mag_tau{int(tau)}'].to_numpy()
...
def fig_definition(ep, lbl, out):
    ...
    ax.plot(t, g['imb_mag_fF'], lw=2.2, color='#B9380B',
    ...
    ax.set_title('3  MAGNITUDE — low pass on the absolute. Always positive, '
                 'slowly varying: how one-sided the imbalance is.',
    ...
    fig.suptitle(f'{lbl} — from a velocity to a night-scale capacitance-imbalance marker',
...
def fig_grid(ep, sessions, out):
    ...
    for ax, lbl in zip(axes.ravel(), sessions):
        ...
        ax2 = ax.twinx()
        ax2.plot(t, g['imb_mag_fF'], lw=1.5, color='#B9380B', alpha=0.85, zorder=3)
        ax2.set_ylabel(f'magnitude ({CAP_UNIT})', fontsize=8, color='#B9380B')
        ax2.tick_params(axis='y', labelcolor='#B9380B', labelsize=7)
        ax2.set_ylim(0, max(np.nanpercentile(g['imb_mag_fF'], 98) * 1.35, 1))
    ...
    fig.suptitle(f'Imbalance magnitude and direction over the night — all 12 sessions '
...
def fig_summary(sess, sweep, ep, out):
    ...
    ax = axes[0, 0]
    ax.bar(xs, s['imb_mag_median_fF'], color='#B9380B', alpha=0.85,
           edgecolor='k', lw=0.5, label='median A(t)')
    ax.plot(xs, s['imb_mag_p90_fF'], 'o', color='#2C3E50', ms=6, label='90th pct')
    for x, v in zip(xs, s['imb_mag_median_fF']):
        ax.annotate(f'{v:.1f}', (x, v), fontsize=7, ha='center',
                    xytext=(0, 4), textcoords='offset points')
    ax.set_xticks(xs); ax.set_xticklabels(s['session'], rotation=45, ha='right')
    ax.set_ylabel(f'imbalance magnitude ({CAP_UNIT})')
    ax.set_title('A  How one-sided is the imbalance, per night', fontsize=11,
    ...
    ax = axes[0, 1]
    w = 0.38
    ax.bar(xs - w / 2, s['imb_dir_first_third'], w, color='#2980B9', alpha=0.85,
           edgecolor='k', lw=0.5, label='first third of the night')
    ax.bar(xs + w / 2, s['imb_dir_last_third'], w, color='#E67E22', alpha=0.85,
    ...
    ax.bar(xs + w / 2, s['imb_dir_persist_acf'], w, color='#1F618D', alpha=0.85,
    ...
    ax = axes[1, 1]
    have = s.dropna(subset=['imb_dir_reversals_supine'])
    xr = np.arange(len(have))
    ax.bar(xr - w / 2, have['imb_dir_reversals'], w, color='#7F8C8D', alpha=0.85,
           edgecolor='k', lw=0.5, label='all epochs')
    ax.bar(xr + w / 2, have['imb_dir_reversals_supine'], w, color='#16A085',
    ...
    fig.suptitle('Night-scale capacitance-imbalance marker — magnitude, direction, timescale, '
...
def fig_vs_headangle(ep, sess, out):
    ...
    ax.scatter(g['turn_deg'], g['imb_dir'], s=4, alpha=0.12, color='#1F618D')
    for lv in (-90, 0, 90):
    ...
    ax.set_ylabel('imbalance direction D')
    ...
    for lbl, gg in sup.groupby('session'):
        ...
        ax.plot(gg['t_hr'], gg['imb_dir'], lw=1.2, alpha=0.75, label=lbl)
    ax.axhline(0, color='k', lw=1.1)
    ax.set_ylim(-1.2, 1.2)
    ax.set_xlabel('Time (hours)'); ax.set_ylabel('imbalance direction D')
    ...
    data, labels = [], []
    for p_ in order:
        v = g.loc[g['head_pos'] == p_, 'imb_mag_fF'].dropna().to_numpy()
        if len(v) >= 30:
    ...
    ax.set_ylabel(f'imbalance magnitude A(t)  ({CAP_UNIT})')
    ...
    fig.suptitle('The control: how much of the capacitance-imbalance marker is the head turning?',
...
def main():
    ...
    ep.to_csv(REPORT_DIR / 'imbalance_epochs.csv', index=False)
    sess.to_csv(REPORT_DIR / 'imbalance_session.csv', index=False)
    sweep.to_csv(REPORT_DIR / 'imbalance_tau_sweep.csv', index=False)

    sessions = sorted(ep['session'].unique())
    demo = DEMO if DEMO in sessions else sessions[0]
    fig_definition(ep, demo, FIG_DIR / 'imbalance_definition.png')
    fig_grid(ep, sessions, FIG_DIR / 'imbalance_grid.png')
    fig_summary(sess, sweep, ep, FIG_DIR / 'imbalance_summary.png')
    fig_vs_headangle(ep, sess, FIG_DIR / 'imbalance_vs_headangle.png')

    print('=' * 104)
    print(f'Night-scale capacitance-imbalance marker  (low pass = rolling median over {TAU_MIN:.0f} min, '
          f'motion epochs excluded)')
    print('=' * 104)
    show = sess[['session', 'session_mean_fF', 'imb_mag_median_fF', 'imb_dir_median',
                 'imb_dir_first_third', 'imb_dir_last_third', 'imb_dir_reversals',
    ...
    print(f"  direction D(t)  : median {sess['imb_dir_persist_acf'].median():+.3f}  "
          f"[{sess['imb_dir_persist_acf'].min():+.3f}, "
          f"{sess['imb_dir_persist_acf'].max():+.3f}]"
    ...
    sp = sess.dropna(subset=['imb_dir_median_supine'])
    print(f"  supine-only: {len(sp)} sessions with >=30 supine epochs; "
          f"median magnitude {sp['imb_mag_median_supine_fF'].median():.1f} {CAP_UNIT} "
          f"vs {sp['imb_mag_median_fF'].median():.1f} {CAP_UNIT} over all epochs;")
    print(f"               direction reversals {sp['imb_dir_reversals_supine'].median():.1f} "
          f"(supine) vs {sp['imb_dir_reversals'].median():.1f} (all) -- the marker is "
