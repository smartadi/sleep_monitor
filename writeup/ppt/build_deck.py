...
SLIDES = [
    ...
    *per_night("channel_evolution/%s_CH_CLE-CRE.png", "Sensor value — %s",
    ...
    (None, "Variance as a sleep-depth indicator",
     "Low variance is deep sleep, high is wake; N3 lowest in every subject (AUC ~0.65, not a full stager).",
     "staging/variance_staging.png"),
    (None, "Variance around REM onset",
     "It steps up at REM onset without an anticipatory rise in the minutes before.",
     "staging/variance_peri_rem.png"),
    ...
    (None, "Choosing the variance threshold",
     "A per-night percentile puts the same 10% above threshold every night, by construction.",
     "prof_metrics/threshold_policy.png"),
    ...
    *per_night("prof_metrics/arousal_timeseries/%s.png",
               ...
               "Hypnogram, event lanes with smoothed density, capacitive variance, slow mean, motion."),
    (None, "Do mask bumps land where EEG arousals cluster?",
     "Episodes detected in each signal on its own, then asked whether they coincide. Roughly twice chance.",
     "prof_metrics/bump_matching_simple.png"),
    (None, "The same night by night",
     "Each night's two curves with its episodes marked — ▲ mask, ▼ EEG.",
     "prof_metrics/bump_matching_nights.png"),
