...
- **Added** `analysis/spindles/plot_spindle_windows.py` — single-event spindle windows: for the N strongest N2 spindles in a session, plots EEG raw + EEG 11–16 Hz + CAP CLE−CRE raw + CAP 11–16 Hz + CAP sigma spectrogram over a shared ±3 s window with the scored spindle span shaded. Shows event-level (not just averaged) that CAP carries no sigma. Output `outputs/fig_spindle_windows_<label>.png`.
- **Added** `analysis/spindles/spindle_indirect.py` — two follow-up tests over all 12 sessions. #3 heart-rate route: spindle-triggered instantaneous HR from ECG R-peaks (positive control) vs CAP cardiac-band pulse peaks, ±15 s, with a random-N2 null. #2 coherence route: whole-night magnitude-squared coherence EEG↔CAP CLE−CRE (leakage test) and CLE↔CRE (same-source anchor). Writes `spindle_hr_route.csv`, `spindle_coherence.csv`, `spindle_hr_triggered.npz`, `spindle_coherence.npz`.
- **Added** `analysis/spindles/plot_indirect.py` — figures for the two routes: `fig_spindle_hr_triggered.png` (ECG shows biphasic ±~1.4 bpm spindle-cardiac beat, CAP flat vs null) and `fig_spindle_coherence.png` (EEG-CAP coherence ≈0 incl. sigma; CLE-CRE anchor peaks at resp/cardiac, proving the estimator works).
