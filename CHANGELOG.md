...
- **Changed** `analysis/mean_value/channel_evolution.py` — (1) baseline velocity is now computed on a finer **1 Hz** baseline (so a sub-Nyquist frequency cap is meaningful) and **low-pass filtered at 0.05 Hz** (Butterworth-4, zero-phase) after the accelerometer regression: velocity = d/dt of the sub-0.05 Hz motion-regressed baseline (user asked for a 0.15 Hz cap; 0.15 Hz left the differentiated trace dominated by amplified noise at ±10⁴–10⁵ a.u./hr, so per user decision the cap was lowered to 0.05 Hz). (2) Mean-value row (B) uses an **adaptive, median-centred y-axis** (median ± 5·MAD, clamped to never zoom out past the data) so sessions with huge coupling swings (S6) still show the structure around the baseline while extreme excursions clip, instead of squishing the trace.
